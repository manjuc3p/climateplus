...
class Operations extends CI_Controller
{
	...
	function daily_job_list()
	{
		...
		log_message('debug', 'Date: ' . print_r($data['job_date'], true));
		$this->load->model('Operations_model');
	...
	function all_job_list()
	{
		...
		$jobs = $this->Operations_model->get_all_job_list($data['job_date'], 1, $dep, $usr);
		...
		if ($print == 1) {
			...
			$data['main_content'] = 'operations/all_job_list.php';
			$this->load->view('includes/template.php', $data);
		}
	}
	...
	function pending_job_list()
	{
		...
		$jobs = $this->Operations_model->get_pending_job_list($data['job_date'], 1, $dep, $usr);
		...
		if ($print == 1) {
			...
			$data['main_content'] = 'operations/pending_job_list.php';
			$this->load->view('includes/template.php', $data);
		}
	}
	...
	function pending_jobs()
	{
		...
		if (isset($_POST['job_date'])) {
			$data['job_date'] = $_POST['job_date'];
			$data['records'] = $this->Operations_model->get_pending_job_list($dep, $usr, $_POST['job_date']);
		} else {
			$data['records'] = $this->Operations_model->get_pending_job_list($dep, $usr, '');
		}

		$data['main_content'] = 'operations/pending_jobs.php';
		$this->load->view('includes/template.php', $data);
	}
	...
	function print_esr()
	{
		...
		$this->load->view('operations/print/print_esr.php', $data);
	}

	function generate_esr($job_id)
	{
		$data['job_id'] = $job_id;
		$this->load->model('Operations_model');
		$data['job'] = $this->Operations_model->get_job_by_id($data['job_id']);
		if ($data['job']['job_type'] == 1) {
			$data['report_title'] = 'DELIVERY ORDER';
		} else
			$data['report_title'] = 'ENGINEERING SERVICE REPORT';
		$data['selected_options'] = $this->Operations_model->get_checklist_options_for_job($data['job_id']);
		$data['new_options'] = $this->Operations_model->get_new_checklist_options_for_job($data['job_id']);
		...
		$file_path = FCPATH . 'public/uploded_documents/esr/' . $file_name;
		$pdfOutput = $dompdf->output();

		// Save file
		file_put_contents($file_path, $pdfOutput);
