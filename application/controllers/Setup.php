...
defined('BASEPATH') OR exit('No direct script access allowed');
  ...
  function add_job_type(){
	...
	$data['main_content']='setup/add_job_type.php';
	$this->load->view('includes/template.php',$data);
  }

  function insert_job_type(){

	$this->load->model('Setup_model');
	$res=$this->Setup_model->insert_job_type();
	if($res!='')
		$this->session->set_flashdata('success', 'Data Updated Successfully..');
	else
		$this->session->set_flashdata('warning', 'Something went wrong..');

	redirect('Setup/list_job_types');
  }
  ...
  function edit_job_type(){
	...
	$this->load->model('Setup_model');
	$data['job_type']=$this->Setup_model->get_job_type_by_id($id);
	
	$data['main_content']='setup/edit_job_type.php';
	$this->load->view('includes/template.php',$data);
  }

  function update_job_type(){
	$this->load->model('Setup_model');
	$res=$this->Setup_model->update_job_type();
	if($res!='')
		$this->session->set_flashdata('success', 'Data Updated Successfully..');
	else
		$this->session->set_flashdata('warning', 'Something went wrong..');
