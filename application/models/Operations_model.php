...
class Operations_model extends CI_Model
{
	...
	function get_daily_job_list($job_date, $job_type, $dep = '', $usr = 0)
	{
		...
		//  log_message('debug', 'Job Query Result: ' . print_r($res, true));
		return $res;
	}

	function get_all_job_list($job_date, $job_type, $dep = '', $usr = 0)
	{
	...
	// function get_pending_job_list($job_date, $job_type, $dep = '', $usr = 0)
	// {
	...
	function update_job()
	{
		...
		$data = array(
			...
			'document_comment' => $this->input->post('doccomments'),
			'additional_notes' => $this->input->post('addnotes'),
		);

		// if ($_POST['new_job_date'] != '') {
		// 	$data['new_job_date'] = $_POST['new_job_date'];
		// }
		$new_job_date = $this->input->post('new_job_date');

		if (!empty($new_job_date)) {
			$data['new_job_date'] = $new_job_date;   // admin case
		} else {
			$data['new_job_date'] = $data['new_job_date'] ?? '';  // non-admin case
		}
	...
	function get_pending_job_list($dep, $user, $job_date = '')
	{
		$this->db->select('jm.*,jt.report_abbr,cm.cust_name,u1.user_name as sales,u2.user_name as staff');
		$this->db->from('job_master jm');
		$this->db->join('job_types jt', 'jm.job_type = jt.id', 'left');
