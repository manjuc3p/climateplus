    ...
    <div class="col-sm-12">
      <div class="card">
        <div class="card-header table-card-header">
          <h5>Approved Quotation Report</h5>
        </div>
        <div class="card-block">
          <form class="form-horizontal" action="<?php echo base_url().'index.php/'; ?>Reports/get_approved_quotation_report" id="question" method="post" name="question" >
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-2">
                <table>
                  ...
                  <td>&nbsp;</td>
                  <td>
                    <form target="_blank" action="<?php echo base_url().'index.php/'; ?>Reports/print_approved_quotation_report" id="ques1" method="post" name="ques1" >
                    ...
                    <td>&nbsp;</td>
                    <td>
                      <form action="<?php echo base_url().'index.php/'; ?>Reports/export_approved_quotation_report" id="ques1" method="post" name="ques1" >
                        <input type="hidden" name="from" value="<?php echo $from; ?>" />
                        <input type="hidden" name="to" value="<?php echo $to; ?>" />
                        <input type="hidden" name="customer_id" value="<?php echo $customer_id; ?>" />
                        <input type="hidden" name="status" value="<?php echo $status; ?>" />
                ...
                <tbody>
                  <?php $i=1; foreach($records as $row) :?>
                    <tr>
                      ...
                      <td><?php if($row->po_uploaded=='Yes' && $row->Po_file!='') { ?>
                        <u><a target="blank" href="<?php echo base_url().$row->Po_file;?>" download>View File</a></u>
                      <?php } else {?>
                        <!--<a target='blank' href="<?php echo base_url().'index.php/Sales/upload_client_po/'.$row->quot_id.'/'.$row->revision;?>" title="Upload PO" class="badge badge-info">Upload PO</a>-->
                      <?php } ?>
                    </td>
                    <td>
                      <?php  $ev=$row->revision;
                      for($k=1; $k <= $ev; $k++)
                      {?>
                        <u><a href="<?php echo base_url().'index.php/Sales/edit_quotation/'.$row->quot_id.'/'.$k.'/0';?>" title="View Revision" >Revision <?php echo $k; ?></a></u><br>
                      <?php }
                      ?>
                    </td>
