
<html>
	<head>
		<title>
			ESR
		</title>
	</head>
	<body style="font-family:Arial;">
		<center>
		<img src="<?= FCPATH .'public/header_img/cp_headr.png' ?>" alt='logo.png' width='1024px'>
		</center>			
	
		<main>
		<center>
			<table cellpadding=10 width=100%>
                <tr style='height:22px'>
                    <td bgcolor="#0394fc" align="center" style='font-size:30px;font-color:#ffffff'>
                         ENGINEERING SERVICE REPORT
                    </td>
                </tr>
			</table>
			<table cellpadding=10 width='100%' style='font-size: 12px;border:1px solid #000;'>
            		<tr>
						<td width='50%'>
							<table>
								<tr>
									<td>Customer:</td><td><b><?php echo $job['customer'];?></b></td>
								</tr>
								<tr>
									<td>Location:</td><td><b><?php echo $job['job_location'];?></b></td>
								</tr>
								<tr>
									<td>Contact:</td><td><b><?php echo $job['contact'];?></b></td>
								</tr>
                                <tr>
									<td>Time:</td><td><b><?php echo date("g:i A", strtotime($job['job_start_time'])).'-'.date("g:i A", strtotime($job['job_end_time']));?></b></td>
								</tr>						
							</table>
						</td>
						
						<td width='50%'>
							<table>
									<tr>
										<td>Date:</td><td><b><?php echo $job['new_job_date']??$job['job_date'];?></b></td>
									</tr>
									<tr>
										<td>Invoice No:</td><td><b>123456</b></td>
									</tr>
                                    <tr>
										<td>Engineer:</td><td><b><?php echo $job['staff'];?></b></td>
									</tr>
                                    <tr>
										<td>Service Team:</td><td><b></b></td>
									</tr>
							</table>
						</td>
						
					</tr>
					<tr style='height:22px'>
                    <td  align="center" style='font-size:30px;font-color:#ffffff'>
                         <BR>
                    </td>
                	</tr>
					<tr style='height:10%'>
                        <td width='20%'>JOB DESCRIPTION</td>
                        <td width='80%'><?= $job['job_desc'] ?><BR></td>
                    </tr>
					<tr>
							<td width='20%'>CHECKLIST</td>
							<td width='80%'><?= in_array(1, $selected_options) ? '✔' : '☐' ?>Leakages<br>
											<?= in_array(2, $selected_options) ? '✔' : '☐' ?>Is water pump working<br>
											<?= in_array(3, $selected_options) ? '✔' : '☐' ?>Remote Control Batteries<br>
											<?= in_array(4, $selected_options) ? '✔' : '☐' ?>Scratches<br>
											<?= in_array(5, $selected_options) ? '✔' : '☐' ?>Are wheels properly tightened<br>
											<?= in_array(6, $selected_options) ? '✔' : '☐' ?>Bad Smell<br>
											<?= in_array(7, $selected_options) ? '✔' : '☐' ?>Are all buttons working<br>

										</td>
							
					</tr>
					<tr>
                        
                        <td width='20%'>Others</td>
                        <td width='80%'>
							
							<?php foreach($new_options as $new){?>
							<?php echo '✔'.$new->checklist_option; ?><br>
							<?php } ?>
                    	</td>
                    </tr>
					<tr>
                    <td width='20%'>WORK CARRIED OUT</td>
                    <td width='80%'>
						<?php echo $job['work_done']; ?><br>
						<?php if($job['job_photo'] !=''){ ?>
						<img src="<?= FCPATH . 'public/uploded_documents/job_photos/' . $job['job_photo']; ?>" height='40%' width='40%' alt="Sample Image">
						<?php }?>
						<br>	
					</td>
                    </tr>
					<tr>
                    <td width='20%'>PARTS</td>
                    <td width='80%'><?php echo $job['parts']; ?><br></td>    
                    </tr>
					<tr style='height:22px'>
                    <td  align="center" style='font-size:30px;font-color:#ffffff'>
                         <BR>
                    </td>
                	</tr>
					<tr>
                        <td>Customer's Sign:</td>
                        <td><img src="<?php echo base_url($job['customer_sign']);?>" height='50px' width='50px' ></td>
                    </tr>
						
					
            </table>
			<br>
            <!-- <table cellpadding=10 width='100%' style='font-size: 12px;border:1px solid #000;'>
			
                    <tr style='height:10%'>
                    <td width='20%'>WORK CARRIED OUT</td>
                    <td width='80%'>
						<table width='100%'>
							<tr>
									<td><?php echo $job['work_done']; ?></td>
									<?php if($job['job_photo'] !=''){ ?>
									<td width=50%>
									<img src="<?= FCPATH . 'public/uploded_documents/job_photos/' . $job['job_photo']; ?>" height='200px' width='300px' alt="Sample Image">
									</td>
									<?php }?>
							</tr>
						</table>
					</td>
                    </tr>
                    
                   
			</table>  -->
		</center>
		</main>
	
	</body>
</html>








