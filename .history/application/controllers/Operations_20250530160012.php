<?php date_default_timezone_set('Asia/Dubai');
	...
	function generate_esr($job_id){
		...
		$headerPath    = base_url() . "public/header_img/cp_headr.png";
		$canvas->page_script(function ( $canvas, $fontMetrics) use ($font,$headerPath) {
			
			$canvas->image($headerPath, 25, 0, 550, 100); 
		});
		$file_name = $data['job']['job_title'].'.pdf';
		$file_path = FCPATH . 'public/uploded_documents/esr/'.$file_name;
		file_put_contents($file_path, $this->dompdf_gen->dompdf->output());
