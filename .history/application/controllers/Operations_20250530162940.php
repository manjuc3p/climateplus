...
$headerPath = FCPATH . "public/header_img/cp_headr.png";

// Add image to each page
$canvas->page_script(function ($page_canvas, $fontMetrics, $page_number, $page_count) use ($font, $headerPath) {
    if (file_exists($headerPath)) {
        $page_canvas->image($headerPath, 25, 0, 550, 100);
    }
});
...
// ✅ Use the correct Dompdf instance to output
file_put_contents($file_path, $dompdf->output());

return 'public/uploded_documents/esr/' . $file_name;
		//$dompdf->stream("report_$job_id.pdf", array("Attachment" => false));
		// $pdf = new Tcpdf();
