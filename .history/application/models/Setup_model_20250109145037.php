<?php
	...
	function insert_job_type(){
		...
		$this->db->insert('job_types',['job_type'=>$type]);
		...
		$ci = get_instance();
		$ci->load->helper('log');
		$log_msg=add_log_entry($user_se_id,2,$page_name[1],'job_types','id',$id);
		return true;
		}
		else
		{
	...
	function get_job_type_by_id($id){
		$this->db->select('*');
		$this->db->where('id',$id);
		$res = $this->db->get('job_types')->row_array();
		return $res;
	}
	...
	function update_job_type() 
	{
		...
		$this->db->where('id',$id);
		$res = $this->db->update('job_types', $data);
